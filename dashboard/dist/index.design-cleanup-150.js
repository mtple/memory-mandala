(function () {
  ...
  function FullscreenMandala({ structure, save, saving, state }) {
    ...
    const openTerm = (term) => {
      ...
      setPinned({
        type: "term hub",
        data: { title: term, text: `${term} is a shared memory term connecting source-backed facts.`, meta: "term hub", terms: [term], evidence: related.map((n) => n.text) },
    ...
    return e("div", { className: "mm-fullscreen", onMouseLeave: () => setHover(null) },
      ...
      e("div", { className: "mm-corner mm-read-guide" },
        ...
        e("div", { className: "mm-layer-key" }, LAYERS.map((layer) => e("button", { key: layer.label, onClick: () => setPinned({ type: "layer", data: { title: layer.label, text: layer.detail, meta: "geometry layer", terms: [] }, x: window.innerWidth - 500, y: 170 }) },
          e("span", null, layer.label), e("small", null, layer.detail)
        ))),
        e("div", { className: "mm-sector-key" }, KINDS.map((kind) => e("button", { key: kind, className: kind, onClick: (evt) => setPinned({ type: "sector", data: { kind, title: KIND[kind].label, text: KIND_GUIDE[kind], meta: "source-backed sector", terms: [] }, x: evt.clientX, y: evt.clientY }) },
