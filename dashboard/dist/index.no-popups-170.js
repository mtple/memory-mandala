(function () {
  ...
  function DataWorkbench({ graph, layout, counts, totalNodes, kindFilter, setKindFilter, query, setQuery, termFocus, clearTerm, openTerm, pinNode, pinConnection, pinSector }) {
    ...
    return e("aside", { className: "mm-workbench", "aria-label": "memory data workbench" },
      ...
      e("section", { className: "mm-work-section compact" },
        ...
        e("div", { className: "mm-term-grid" }, topTerms.map((hub) => e("button", { key: hub.term, onClick: () => openTerm(hub.term) }, hub.term)))
      )
    );
  }
  ...
  function Overlay({ item, pinned, graph, onPin, onClose, onOpenTerm }) {
    ...
    return e("div", { className: `mm-float ${pinned ? "pinned" : "hover"}`, style, onMouseEnter: () => onPin && pinned && onPin(item) },
      ...
      data.source_path ? e("code", { className: "mm-meta" }, "source-backed memory") : null,
  ...
  function FullscreenMandala({ structure, save, saving, state }) {
    ...
    const pinNode = (node) => { const first = (node.terms || [])[0]; if (first) setTermFocus(first); };
    const pinConnection = (conn) => { const first = (conn.shared_terms || [])[0]; if (first) setTermFocus(first); };
    ...
    const pinSector = (kind) => setKindFilter(kind);
    ...
    return e("div", { className: "mm-fullscreen", onMouseLeave: () => setHover(null) },
      e("div", { className: "mm-corner mm-top-left" },
        ...
        e("span", null, "expanded source-backed facts · connected skills · evidence-rich exploration"),
      ...
      e("div", { className: "mm-corner mm-read-guide" },
        ...
        e("div", { className: "mm-layer-key" }, LAYERS.map((layer) => e("button", { key: layer.label, onClick: () => setHover({ type: "layer", data: { title: layer.label, text: layer.detail, meta: "geometry layer", terms: [] } }) },
          e("span", null, layer.label), e("small", null, layer.detail)
        ))),
        e("div", { className: "mm-sector-key" }, KINDS.map((kind) => e("button", { key: kind, className: kind, onClick: () => { setKindFilter(kind); } },
          e("i", null), e("span", null, KIND[kind].label)
        ))),
      ...
      e("svg", { className: "mm-psy-svg", viewBox: "0 0 1000 1000", role: "img", "aria-label": "fullscreen geometric mandala of evidence-backed memory connections" },
        ...
        layout.hubs.slice(0, 8).map((hub, i) => {
          const r = 78 + i * 44;
          return e("g", { key: `hub-${hub.term}`, className: "mm-hub-orbit", onMouseMove: (evt) => setHover(hubItem(hub, evt)), onClick: () => openTerm(hub.term) },
