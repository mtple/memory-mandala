(function () {
  ...
  function DynamicInsightPane({ detail, graph, layout, structure, state, saving, save, onOpenTerm, onSelectNode, onSelectConnection, onSelectSector, clearFilters }) {
    ...
    const title = data ? detailTitle(detail) : "memory graph";
    ...
    return e("div", { className: `mm-corner mm-read-guide mm-dynamic-pane ${kind || "overview"}`, "data-mode": mode },
      e("p", null, "right pane follows hover"),
      e("div", { className: "mm-dynamic-head" },
        e("span", { className: `mm-dynamic-kind ${kind || "overview"}` }, mode),
        e("h2", null, short(title, 74)),
        data && data.meta ? e("small", null, data.meta) : e("small", null, "move across the mandala to inspect live memory structure")
      ),
      data && data.text ? e("p", { className: "mm-dynamic-text" }, data.text) : e("p", { className: "mm-dynamic-text" }, "hover sectors, hubs, links, and memory glyphs. this pane transforms into the active evidence view without opening a popup."),
  ...
  function FullscreenMandala({ structure, save, saving, state }) {
    ...
    const pinNode = (node) => { setSelected({ type: "memory fact", data: { ...node, title: node.source, meta: (node.terms || []).join(", "), source_path: node.source_path } }); const first = (node.terms || [])[0]; if (first) setTermFocus(first); };
